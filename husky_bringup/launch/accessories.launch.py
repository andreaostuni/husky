...
def generate_launch_description():
    ...
    primary_lidar_enable = EnvironmentVariable(
        'CPR_LASER', default_value='false')
    primary_lidar_model = EnvironmentVariable(
        'CPR_LASER_MODEL', default_value='ust10')
    ...
    primary_lidar_3d_enable = EnvironmentVariable(
        'CPR_3D_LASER', default_value='false')
    ...
    if (primary_lidar_3d_enable.perform(lc)) == 'true':
        if (primary_lidar_3d_model.perform(lc) == 'vlp16'):
            ...
            velodyne_pointcloud_dir = ament_index_python.packages.get_package_share_directory(
                'velodyne_pointcloud')

            velodyne_pointcloud_params_file = os.path.join(
                velodyne_pointcloud_dir, 'config', 'VLP16-velodyne_convert_node-params.yaml')

            with open(velodyne_pointcloud_params_file, 'r') as f:
                config_velodyne_pointcloud_vlp16_params = yaml.safe_load(
            ...
            velodyne_pointcloud_params_file = os.path.join(
                velodyne_pointcloud_dir, 'config', 'VLP32C-velodyne_convert_node-params.yaml')

            with open(velodyne_pointcloud_params_file, 'r') as f:
                config_velodyne_pointcloud_vlp32c_params = yaml.safe_load(
                    f)['velodyne_convert_node']['ros__parameters']
    ...
    primary_imu_baud = EnvironmentVariable(
        'CPR_IMU_BAUD', default_value='115200')
    primary_imu_mount = EnvironmentVariable(
        'CPR_IMU_MOUNT', default_value='imu_link')
